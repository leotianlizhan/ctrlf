...
import com.zhan.leo.ctrlf.MainActivity;
...
public class CameraSource {
    ...
    // it's the actual physical size of text on screen that matters
    // evidence: zooming in recognizes successfully
    private int mRequestedPreviewWidth = 1920;  //1024          1280
    private int mRequestedPreviewHeight = 1080;  //768           960
    ...
    public Camera.Size getOptimalSize(Camera mCamera) {
        List<Camera.Size> list = mCamera.getParameters().getSupportedPreviewSizes();
        Log.d("****preview count", Integer.toString(list.size()));
        for (Camera.Size s: list) {
            Log.d("**PREVIEW SIZE: ", Integer.toString(s.width)+", "+Integer.toString(s.height));
        }
        return list.get(0);
    }
    public void setOptimalSize(Camera mCamera) {
        List<Camera.Size> list = mCamera.getParameters().getSupportedPreviewSizes();
        Log.d("****preview count", Integer.toString(list.size()));
        if(list.isEmpty()) return;
        Camera.Size first = list.get(0);
        Camera.Size last = list.get(list.size()-1);
        Camera.Size optimal = first.width >= last.width ? first : last;
        Toast.makeText(mContext, Integer.toString(optimal.width)+", "+Integer.toString(optimal.height), Toast.LENGTH_LONG).show();
        for (Camera.Size s: list) {
            Log.d("**PREVIEW SIZE: ", Integer.toString(s.width)+", "+Integer.toString(s.height));
        }
        //mRequestedPreviewWidth = optimal.width;
        //mRequestedPreviewHeight = optimal.height;
    }
    ...
    private Camera createCamera() {
        ...
        Log.e("finalsize", Integer.toString(mPreviewSize.getWidth()) + " x " + Integer.toString(mPreviewSize.getHeight()));
    ...
    private static List<SizePair> generateValidPreviewSizeList(Camera camera) {
        ...
        for (android.hardware.Camera.Size previewSize : supportedPreviewSizes) {
            ...
            Log.e("aaaaabbb", Integer.toString(previewSize.width) + " x " + Integer.toString(previewSize.height));
            // By looping through the picture sizes in order, we favor the higher resolutions.
            // We choose the highest resolution in order to support taking the full resolution
            // picture later.
            for (android.hardware.Camera.Size pictureSize : supportedPictureSizes) {
                float pictureAspectRatio = (float) pictureSize.width / (float) pictureSize.height;
                //TODO: testing different resolution results
